...
class GaussDB(Postgres):  # 继承自 PostgreSQL 方言
    ...
    class Parser(Postgres.Parser):
        ...
        def _parse_when_matched(self) -> exp.Whens:
            """
            Override merge WHEN parsing so GaussDB only treats a column tuple when it
            actually sees a ROW keyword or leading parenthesis. This keeps the default
            INSERT VALUES (...) form from consuming the VALUES token as part of the
            column list.
            """
            whens = []
